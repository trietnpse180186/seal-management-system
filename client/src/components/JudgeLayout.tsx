...
export default function JudgeLayout({ user, onLogout }: JudgeLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#070d1f] text-slate-300 flex font-sans selection:bg-indigo-500/30">
      {/* SideNavBar */}
      <aside className="fixed left-0 top-0 h-full w-[280px] bg-slate-900/40 backdrop-blur-2xl border-r border-white/5 flex flex-col z-20 shadow-2xl">
        ...
        <div className="p-6 border-b border-white/5 flex flex-col items-center relative overflow-hidden">
          ...
          <div className="w-16 h-16 bg-indigo-500/10 rounded-full border border-indigo-500/30 flex items-center justify-center text-indigo-400 mb-3 shadow-[0_0_15px_rgba(99,102,241,0.2)]">
            <Award size={32} />
          </div>
          ...
          <p className="text-[10px] text-indigo-400 font-semibold uppercase tracking-widest mt-1">Hackathon Edition</p>
        </div>

        {/* User Quick Info */}
        <div className="px-6 py-4 border-b border-white/5 flex items-center gap-3 bg-slate-900/20">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 text-white flex items-center justify-center font-bold text-sm shadow-[0_0_10px_rgba(99,102,241,0.4)]">
            {user?.fullName?.charAt(0) || 'J'}
          </div>
          <div className="overflow-hidden">
            <h4 className="text-xs font-bold text-white truncate">{user?.fullName || 'Judge Name'}</h4>
            <p className="text-[9px] text-violet-400 font-mono uppercase tracking-wider">Vai trò: Giám khảo</p>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-300 relative overflow-hidden ${
                  active
                    ? 'bg-indigo-500/10 text-indigo-300 border border-indigo-500/30 shadow-[inset_0_0_20px_rgba(99,102,241,0.1)]'
                    : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/30 border border-transparent hover:border-white/5'
                }`}
              >
                {active && (
                  <div className="absolute left-0 top-0 h-full w-[3px] bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]"></div>
                )}
      ...
      <div className="flex-1 ml-[280px] flex flex-col min-h-screen relative">
        ...
        <header className="h-16 w-full px-8 bg-slate-950/60 backdrop-blur-xl border-b border-white/5 flex justify-between items-center z-10 sticky top-0 shadow-lg">
          <div className="flex items-center gap-3">
            <span className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-400 text-sm tracking-widest uppercase drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]">
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              <button
                ...
                className="text-slate-400 hover:text-indigo-300 transition-colors relative p-1.5 rounded-full hover:bg-indigo-500/10"
              >
                <Bell size={18} />
                {unreadCount > 0 && (
            ...
            <div className="flex items-center gap-3 border-l border-white/10 pl-6">
              ...
              <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-indigo-400 font-bold text-xs shadow-sm">
