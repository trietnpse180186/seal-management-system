...
export default function OperationsTab({
  ...
  const handleToggleManualExam = async (round: any) => {
    ...
    try {
      ...
      toast.success(newManualStatus ? "Đã mở vòng & đề bài thành công!" : "Đã đóng vòng & đề bài!");
    } catch (err) {
      console.error(err);
    } finally {
            ...
            {selectedRound ? (
              <div className="space-y-6 font-sans">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 bg-slate-950/40 p-4 rounded-xl border border-slate-900 text-xs space-y-2">
                    ...
                    <div className="flex justify-between border-t border-slate-850/60 pt-2">
                      <span className="text-slate-500 font-mono">Trạng thái đề bài:</span>
                      <span className="font-bold">
                        {selectedRound.isExamManualOpen ? (
                          <span className="text-emerald-400">Đang mở (Thủ công)</span>
                        ) : selectedRound.status === "active" ? (
                          <span className="text-emerald-400">Đang mở (Theo lịch)</span>
                        ) : (
                          <span className="text-slate-450">Đang khóa / Chờ</span>
                        )}
                      </span>
                    </div>
                    {(() => {
                ...
                <div className="space-y-4">
                  ...
                  {!readOnly ? (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                      ...
                      <button
                        ...
                        disabled={updatingRoundMap[selectedRound._id] || !isEventOngoing || isLockedByLaterRound}
                      ...
                      {selectedRound.status === 'completed' ? (
                        ...
                        <button
                          ...
                          disabled={updatingRoundMap[selectedRound._id] || !isEventOngoing || isLockedByLaterRound || selectedRound.status === 'pending'}
                      ...
                      <button
                        ...
                        disabled={updatingRoundMap[selectedRound._id] || selectedRound.status !== 'completed' || !isEventOngoing || isLockedByLaterRound}
                      ...
                      >
                        {selectedRound.advanceTopN === 0 ? "KẾT THÚC VÒNG ĐẤU" : "BẮT ĐẦU VÒNG KẾ TIẾP"}
                      </button>
                  ...
                  {!readOnly && rounds.some(r => r.order < selectedRound.order) && (
                        ...
                        onClick={async () => {
                          ...
                          if (isConfirmed) {
                            ...
                            try {
                              ...
                              toast.success("Đã thu hồi vòng đấu thành công!");
                              ...
                              console.error(err);
                              toast.error(err.response?.data?.message || "Lỗi khi thu hồi vòng đấu.");
                            } finally {
                              setUpdatingRoundMap(prev => ({ ...prev, [selectedRound._id]: false }));
                            }
                          }
                  ...
                  <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-900 text-[11px] text-slate-400 space-y-2">
                    <div className="flex gap-2">
                      <AlertTriangle size={14} className="text-emerald-400 shrink-0" />
                      <p>
                        <strong className="text-slate-350">Lưu ý mở vòng &amp; đề bài:</strong> Mở vòng &amp; đề bài thủ công sẽ chuyển trạng thái Vòng đấu thành <span className="text-emerald-400 font-bold">hoạt động</span> và làm tệp đề, tài liệu hướng dẫn xuất hiện ngay lập tức trong Khu vực Đội thi của tất cả các học sinh mà không cần đợi thời gian bắt đầu cấu hình.
                      </p>
                    </div>
                    <div className="flex gap-2 border-t border-slate-900 pt-2">
                      <HelpCircle size={14} className="text-cyan-400 shrink-0" />
                      <p>
                        <strong className="text-slate-350">Tiến trình chuẩn:</strong> Mở cổng đăng ký → Đóng cổng → Mở vòng &amp; đề bài → Đồng bộ phân tích AI (nếu cần thiết)  → Khóa điểm &amp; xếp hạng → Bắt đầu vòng kế tiếp.
                      </p>
                    </div>
                    <div className="flex gap-2 border-t border-slate-900 pt-2">
                      <RefreshCw size={14} className="text-red-500 shrink-0 mt-0.5" />
                      <p>
                        <strong className="text-slate-350">Lưu ý thu hồi vòng đấu:</strong> Thu hồi vòng đấu sẽ đóng vòng thi hiện tại để quay ngược về vòng thi trước đó tại thời điểm vừa Khóa điểm &amp; xếp hạng (trạng thái vòng trước trở lại hoạt động để giám khảo có thể sửa điểm), đồng thời tự động đưa các đội đã thăng hạng quay ngược trở lại vòng đấu trước.
                      </p>
                    </div>
                  </div>
