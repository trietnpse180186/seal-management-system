...
// ==========================================
// CLI & INTERACTIVE INPUT PARSING
// ==========================================

function printUsage() {
  console.log(`
Cách sử dụng:
  1. Nhập tương tác (interactive prompt):
     node test-tools/seeding/test_setup_custom_teams_real.js

  2. Truyền tổng số đội (tự động chia đều cho 3 track HOME, FARM, FACTORY):
     node test-tools/seeding/test_setup_custom_teams_real.js 15
     node test-tools/seeding/test_setup_custom_teams_real.js --total=15

  3. Chỉ định chi tiết từng track:
     node test-tools/seeding/test_setup_custom_teams_real.js --home=5 --farm=5 --factory=5
     node test-tools/seeding/test_setup_custom_teams_real.js --tracks 5,5,5

  4. Cờ bổ sung:
     --help, -h       : Hiển thị hướng dẫn sử dụng
     --dry-run        : Chạy mô phỏng tính toán, không ghi DB hay tạo GitHub repo
`);
}
...
function distributeTotal(total) {
  ...
  return {
    home: base + (remainder >= 1 ? 1 : 0),
    farm: base + (remainder >= 2 ? 1 : 0),
    factory: base
  };
}

function parseDistributionString(inputStr) {
  if (!inputStr) return null;
  const cleaned = inputStr.trim();
  
  // Format: "5,5,5" or "5 5 5"
  if (cleaned.includes(',') || cleaned.includes(' ')) {
    const parts = cleaned.split(/[\s,]+/).map(p => parseInt(p, 10)).filter(n => !isNaN(n));
    if (parts.length >= 3) {
      return { home: parts[0], farm: parts[1], factory: parts[2] };
    }
    if (parts.length === 2) {
      return { home: parts[0], farm: parts[1], factory: 0 };
    }
    if (parts.length === 1) {
      return distributeTotal(parts[0]);
    }
  }

  // Format single integer "15"
  const singleNum = parseInt(cleaned, 10);
  if (!isNaN(singleNum) && singleNum >= 0) {
    return distributeTotal(singleNum);
  }
...
async function promptUserForConfig() {
  ...
  console.log('================================================================');
  console.log(' CẤU HÌNH SỐ LƯỢNG ĐỘI THI CHO CUỘC THI');
  console.log('================================================================');
  console.log('Bạn có thể:');
  console.log(' - Nhập tổng số đội (VD: 15 -> tự chia 5 HOME, 5 FARM, 5 FACTORY)');
  console.log(' - Nhập phân bổ 3 track (VD: "5,5,5" hoặc "4,4,2")');
  console.log(' - Nhấn Enter để dùng mặc định (29 đội: 10 HOME, 10 FARM, 9 FACTORY)\n');

  const answer = await question('👉 Nhập số lượng đội thi [Mặc định: 29]: ');
  rl.close();

  if (!answer || answer.trim() === '') {
    return { home: 10, farm: 10, factory: 9 };
  }

  const parsed = parseDistributionString(answer);
  if (!parsed || (parsed.home === 0 && parsed.farm === 0 && parsed.factory === 0)) {
    console.log('⚠️ Không nhận diện được cú pháp hợp lệ, chuyển về mặc định 29 đội (10 HOME, 10 FARM, 9 FACTORY).');
    return { home: 10, farm: 10, factory: 9 };
  }
...
async function getTeamConfig() {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg.startsWith('--total=')) total = parseInt(arg.split('=')[1], 10);
    else if (arg === '--total' && args[i + 1]) { total = parseInt(args[i + 1], 10); i++; }
    else if (arg.startsWith('--home=')) home = parseInt(arg.split('=')[1], 10);
    else if (arg === '--home' && args[i + 1]) { home = parseInt(args[i + 1], 10); i++; }
    else if (arg.startsWith('--farm=')) farm = parseInt(arg.split('=')[1], 10);
    else if (arg === '--farm' && args[i + 1]) { farm = parseInt(args[i + 1], 10); i++; }
    else if (arg.startsWith('--factory=')) factory = parseInt(arg.split('=')[1], 10);
    else if (arg === '--factory' && args[i + 1]) { factory = parseInt(args[i + 1], 10); i++; }
    else if (arg.startsWith('--tracks=')) {
      const parsed = parseDistributionString(arg.split('=')[1]);
      if (parsed) { home = parsed.home; farm = parsed.farm; factory = parsed.factory; }
    } else if (arg === '--tracks' && args[i + 1]) {
      const parsed = parseDistributionString(args[i + 1]);
      if (parsed) { home = parsed.home; farm = parsed.farm; factory = parsed.factory; }
      i++;
    } else if (!arg.startsWith('-') && !isNaN(parseInt(arg, 10)) && total === null && home === null) {
      total = parseInt(arg, 10);
    }
  }

  if (home !== null || farm !== null || factory !== null) {
    return {
      home: home || 0,
      farm: farm || 0,
      factory: factory || 0,
  ...
  if (process.env.NUM_TEAMS) {
    const envTotal = parseInt(process.env.NUM_TEAMS, 10);
    if (!isNaN(envTotal)) {
      return {
        ...distributeTotal(envTotal),
        isDryRun
      };
    }
  }

  // Interactive prompt if no arguments supplied
  const interactiveConfig = await promptUserForConfig();
  return {
    ...interactiveConfig,
    isDryRun
  };
...
async function setupCustomTeamsRealContest() {
  const config = await getTeamConfig();
  const totalTeams = config.home + config.farm + config.factory;

  console.log('\n================================================================');
  console.log(` KHỞI TẠO CUỘC THI THỰC TẾ: ${totalTeams} ĐỘI THI - 3 TRACKS`);
  console.log(` Phân bổ: HOME: ${config.home} đội | FARM: ${config.farm} đội | FACTORY: ${config.factory} đội`);
  if (config.isDryRun) {
    console.log(' [CHẾ ĐỘ DRY-RUN]: Sẽ không ghi DB hay tạo GitHub Repo');
  }
  console.log('================================================================\n');

  if (totalTeams === 0) {
    console.log('⚠️ Tổng số đội thi là 0. Kết thúc.');
    return;
  }

  if (config.isDryRun) {
    console.log('Dry-run hoàn tất. Cấu hình hợp lệ.');
    return;
  }
  ...
  const eventName = `Hackathon Multi-Agent AI x IoT 2026 - Official Qualifier (${totalTeams} Teams)`;
  ...
  const event = new Event({
    ...
    description: `Cuộc thi Hackathon Multi-Agent AI x IoT 2026 Vòng Sơ Khảo ${totalTeams} Đội Thi.`,
  ...
  const round1 = new Round({
    ...
    advanceTopN: Math.min(5, Math.max(1, Math.ceil(totalTeams / 3)))
  });
  await round1.save();
  ...
  if (config.home > 0) {
    const trackHome = new Track({
      ...
      roundId: round1._id,
      name: 'HOME',
      maxTeams: Math.max(config.home, 10),
      advanceTopN: Math.min(3, Math.max(1, Math.ceil(config.home / 3)))
    });
    await trackHome.save();
    tracksDistribution.push({ trackKey: 'HOME', trackDoc: trackHome, count: config.home, prefix: 'home' });
  }
  ...
  if (config.farm > 0) {
    const trackFarm = new Track({
      ...
      roundId: round1._id,
      name: 'FARM',
      maxTeams: Math.max(config.farm, 10),
      advanceTopN: Math.min(3, Math.max(1, Math.ceil(config.farm / 3)))
    });
    await trackFarm.save();
    tracksDistribution.push({ trackKey: 'FARM', trackDoc: trackFarm, count: config.farm, prefix: 'farm' });
  }
  ...
  if (config.factory > 0) {
    const trackFactory = new Track({
      ...
      roundId: round1._id,
      name: 'FACTORY',
      maxTeams: Math.max(config.factory, 10),
      advanceTopN: Math.min(3, Math.max(1, Math.ceil(config.factory / 3)))
    });
    await trackFactory.save();
    tracksDistribution.push({ trackKey: 'FACTORY', trackDoc: trackFactory, count: config.factory, prefix: 'factory' });
  }

  console.log(`  -> Đã tạo các Track: ${tracksDistribution.map(t => `${t.trackKey} (${t.count} đội)`).join(', ')}.`);

  console.log(`\n[4/5] Khởi tạo ${totalTeams} Đội thi & Tạo GitHub Repositories thật trên Organization...`);
  ...
  for (const group of tracksDistribution) {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      console.log(`\n[Đội ${globalTeamIndex}/${totalTeams}] ${teamName} (${teamCode})`);
  ...
  console.log('\n================================================================');
  console.log(` [5/5] HOÀN TẤT THÀNH CÔNG KHỞI TẠO ${createdReposList.length} ĐỘI VÀ ${createdReposList.length} REPOSITORIES!`);
  console.log('================================================================');
...
setupCustomTeamsRealContest().catch(err => {
